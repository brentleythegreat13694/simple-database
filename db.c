...
// Stores our cursor which points to our data like row our page
typedef struct {
    Table* table;
    uint32_t row_num;
    bool end_of_table;
...
Table* db_open(const char* filename) {
    ...
    // 2. Checks if its greater than 0 if so then check how many rows it has
...
// Allocates size of our cursor then sets our cursor struct to table and to set our table to num_rows and set our cursor end_of_table to true
Cursor* table_end(Table* table) {
    Cursor* cursor = malloc(sizeof(Cursor));
    cursor->table = table;
